...
export default function EditExamScreen() {
  ...
  const { selectedExam, fetchExam, updateExam, isLoading } = useExams();
  ...
  const [selectedReminders, setSelectedReminders] = useState<Reminder['type'][]>([]);
    ...
    const loadExam = async () => {
      if (id) {
        try {
          ...
          setProgress(exam.progress);
          setNotes(exam.notes || '');
          setSelectedReminders(exam.reminders.map(r => r.type));
          setIsCompleted(exam.isCompleted);
        } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    const examData: Partial<ExamFormData> = {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          ...
          {/* Progress Slider */}
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Study Progress: {progress}%</Text>
                ...
                {[0, 25, 50, 75, 100].map(value => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      progress === value && styles.quickProgressBtnActive
                    ]}
                    onPress={() => setProgress(value)}
                  >
                    <Text style={[
                      styles.quickProgressText,
                      progress === value && styles.quickProgressTextActive
                    ]}>
                      {value}%
                    </Text>
                  </TouchableOpacity>
          ...
          >
            ...
            <Text style={styles.completedText}>Mark as Completed</Text>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  checkboxActive: {
    backgroundColor: StudyBloomColors.success,
    borderColor: StudyBloomColors.success,
  },
  completedText: {
    fontSize: 15,
    fontWeight: '500',
